...
`relationship_id` is a lowercase-hex 128-bit value created by pairing or by an
authenticated peer relationship reset. `key_epoch` is zero. A reset creates a
fresh relationship ID instead of reusing the identifier with a non-zero epoch.
...
| `peer_features`        | array | optional authenticated feature IDs            |
...
|  12 | `expires_at`           | uint                            |
| 128 | `peer_features`        | increasing uint array, optional |
...
definitions are not hashes of the displayed pairing code or HTTP wrapper. The
optional peer feature list is authenticated by the invitation envelope or
acceptance signature and by the digests that bind the pairing messages. Its
absence means baseline support only. Acknowledgements repeat the advertisement
so an established relationship can learn features added by a client upgrade.
...
names. Their causal meanings differ:

- `hwm_out_data` advertises the highest data descriptor the sender has created.
  It may be ahead of the recipient's data receive watermark while that data is
  queued or unread, so this field never proves rollback.
- `hwm_out_control` advertises the sender's control-chain head. The enclosing
  descriptor and its predecessor digest establish the control sequence the
  recipient may accept. Independent control delivery can leave newer events
  queued, so the advertised head does not add a rollback proof.
- `hwm_in_data` greater than the recipient's data send sequence claims receipt
  of a descriptor the recipient never created and proves local rollback.
  `hwm_in_data` below a signed acknowledgement or refusal whose result the
  recipient has already retained contradicts the peer's earlier committed
  receipt and proves peer rollback. The acknowledgement being processed does not
  supply this proof: it is signed before the sender advances and persists the
  receive watermark, so its own `hwm_in_data` may still name the preceding
  sequence.
- `hwm_in_control` greater than the recipient's control send sequence claims
  receipt of a control descriptor the recipient never created and proves local
  rollback. Control descriptors are not acknowledged, so a lower value alone
  does not prove peer rollback.

A proved contradiction halts the relationship. The client retains the field,
both compared values, the control descriptor sequence and digest, and the
relationship ID. Administrative revocation remains available without advancing
either delivery chain.
...
DUD resets delivery state by replacing the relationship ID while preserving the
local alias, canonical origin, and trust in the two old signing keys.
`key_epoch` remains zero. The operator on each device must consent to the same
signed transcript; `sync` may report reset state but does not consent.

The proposal binds the old relationship, its zero key epoch, and its generation,
both old device IDs, the four chain heads observed by the proposer, the
successor generation, a random 16-byte reset ID, a separate random 16-byte
relationship ID, the proposer's new public identity, the counts of work
activation abandons, the canonical origin, and an expiry. The acceptance binds
the SHA-256 digest of that proposal, the opposite role, that role's new public
identity, its four chain heads, its abandoned-work counts, and an expiry. Both
signatures use the old relationship signing keys, so a reset changes sequencing
without changing who is trusted.

The server compares proposals for one active relationship by lowercase reset ID;
the smaller ID wins until activation. A signed cancellation changes a proposed
reset to `cancelled`; cancellation after activation is forbidden. A cancelled
record may be replaced. Activation is one metadata transaction: create the
successor at exactly `old_generation + 1`, store the accepted transcript and
receipt, revoke every old capability, write a complete old-relationship
tombstone, and mark the old relationship revoked. Identical retries return the
stored result. A different proposal, skipped generation, stale consent, or
one-sided signature fails without mutation.

The new directional relationship secrets are derived independently:

```text
HKDF-SHA256(
  ikm  = old_directional_relationship_secret,
  salt = reset_id,
  info = "dud/v2/relationship-reset|" || new_relationship_id || "|" || direction,
  L    = 32
)
```

Every new identity, capability proof, slot, nonce domain, and descriptor binds
the fresh relationship ID. Each chain starts at sequence zero with this genesis
digest, where `direction` and `chain` are one-byte registered values:

```text
SHA-256("dud/v2/chain-genesis\0" || new_relationship_id || direction || chain)
```

The first descriptor is sequence 1 and names that digest as its predecessor. Old
descriptors, slots, capabilities, completions, consents, and receipts cannot
enter the successor namespace. A client accepts only the immediate successor of
its durable generation and retains up to eight signed activation records with
both chain snapshots, the transcript, receipt, dispositions, and activation
time. Payload plaintext is not audit data.

A reset recovers sequencing between peers whose identities remain trusted.
Device compromise, seed disclosure, or doubt about the peer identity requires
administrative revocation and fresh out-of-band pairing.
...
conformance suite are present. A server with resumable chunk storage and peer
relationship reset advertises `[2, 3, 5, 6, 7, 9, 10, 11, 12]`; one without
resumable chunk storage omits 7.
...
### 11.4 Peer Relationship Reset

Feature 12 serves one endpoint:

```text
POST /v2/relationships/reset
```

The request is deterministic CBOR. Its wrapper is
`{1: action, 2: signed_map, 3: signature, 4: acceptance_map?, 5: acceptance_signature?}`.
Actions are `1 propose`, `2 status`, `3 accept-and-activate`, and `4 cancel`.
Keys 4 and 5 are required only for action 3.

The proposal signed under domain `dud/v2/relationship-reset/proposal\0` is:

| Key | Field                     | Type                        |
| --: | ------------------------- | --------------------------- |
|   1 | version                   | uint, 1                     |
|   2 | old relationship ID       | bytes, 16                   |
|   3 | reset ID                  | bytes, 16                   |
|   4 | new relationship ID       | bytes, 16                   |
|   5 | initiator role            | uint, 0 or 1                |
|   6 | generation                | uint, old generation plus 1 |
|   7 | canonical origin          | text                        |
|   8 | initiator chain snapshot  | four chain-head maps        |
|   9 | old initiator device ID   | bytes, 16                   |
|  10 | old peer device ID        | bytes, 16                   |
|  11 | new initiator device ID   | bytes, 16                   |
|  12 | new initiator signing key | bytes, 32                   |
|  13 | new initiator recipient   | bytes, 1216                 |
|  14 | initiator disposition     | abandoned-work map          |
|  15 | expiry                    | Unix seconds                |
|  16 | old key epoch             | uint, 0                     |

A chain-head map is `{1: name, 2: sequence, 3: digest}`. The ordered names are
`out:data`, `out:control`, `in:data`, and `in:control`; each digest is 32 bytes.
The disposition map uses keys 1 through 8 for queued deliveries, queued
completions, queued control events, unacknowledged deliveries, inbound
transfers, quarantined chains, resumable transfers, and refused Git checkpoints.

The acceptance signed under domain `dud/v2/relationship-reset/acceptance\0` is:

| Key | Field                     | Type                    |
| --: | ------------------------- | ----------------------- |
|   1 | version                   | uint, 1                 |
|   2 | proposal digest           | bytes, 32               |
|   3 | accepting role            | uint, opposite proposer |
|   4 | new accepting device ID   | bytes, 16               |
|   5 | new accepting signing key | bytes, 32               |
|   6 | new accepting recipient   | bytes, 1216             |
|   7 | accepting chain snapshot  | four chain-head maps    |
|   8 | accepting disposition     | abandoned-work map      |
|   9 | expiry                    | Unix seconds            |

Status signs
`{1: 1, 2: old_relationship_id, 3: role, 4: nonce, 5: expiry, 6: canonical_origin}`
under `dud/v2/relationship-reset/status\0`. Cancellation signs
`{1: 1, 2: old_relationship_id, 3: reset_id, 4: role, 5: nonce, 6: expiry, 7: canonical_origin}`
under `dud/v2/relationship-reset/cancellation\0`.

The response is
`{1: phase, 2: proposal, 3: proposal_signature, 4: acceptance?, 5: acceptance_signature?, 6: receipt?, 7: cancellation?, 8: cancellation_signature?}`.
Phases are `1 proposed`, `2 active`, and `3 cancelled`. The active receipt is
deterministic CBOR containing
`{1: 1, 2: reset_id, 3: old_relationship_id, 4: new_relationship_id, 5: generation, 6: transcript_digest, 7: activated_at}`.
Clients verify every receipt field and the digest of both signed messages before
local activation.

Reset state is durable and is available through signed status even after the old
relationship has been revoked. No application request is allowed while local
state records `proposed`, `accepted`, or `activating`. A server or peer without
feature 12 causes a compatibility refusal before transition state is written.

### 11.5 Delivery-scoped chunk storage and unserved object endpoints
...
### 11.6 Capability Recovery and Administration
...
invitation_cbor_len   = 1441 bytes
invitation_digest     = 99a1b3ba36048be002712288eecc82c834a0acb5d1fa557c29b33177d4a2196d
```
...
acceptance_cbor_len     = 2639 bytes
acceptance_map_digest   = 232b1a1d160e482ee7c7770ae52dc2a69b99acbebea805bca7ddf178833ff43e
acceptance_signature    = ceb8cd42cede40941b015e570fd739b492c69ca0938e7f3542048fcca2259718b
                          4e72965a07c97320c5a2a12f9f2f6ff871d3211734ca14bad653cea1a143d0e
```
...
```text
capabilities_cbor   = a40182010202890203050607090a0b0c03a9011a06400000021a00040000031a
                      00278d0004184005190100060407183c081a0c8000000919100004a201020201
capabilities_digest = 23ae49139af2e620be07f6e7a682e2bbc942aab943924f9be1dd628f390865d6
```

### 12.11 Peer Relationship Reset

```text
reset_id                 = a0a1a2a3a4a5a6a7a8a9aaabacadaeaf
new_relationship_id      = c0c1c2c3c4c5c6c7c8c9cacbcccdcecf
proposal_cbor_len         = 1619 bytes
proposal_digest           = 5b3cb09c87f274dd5ef712af3123f47f2ff9562da2a3f3c5da99d8161be93256
acceptance_cbor_len       = 1535 bytes
acceptance_digest         = 4ebf788f1f8129c5e5644b6a564a2fe183acef7663bb1c5e242449d43fcfa58b
transcript_digest         = 5fb85391fb7da45329348e3d84720f8953520a8af1dbc1cc2175e447bd00e8c1
activation_receipt_digest = 781c8ea8d60334c3c01d90d8b15d0594a4909cf884659cdc393a4861bfe2dd43
relationship_secret[0]    = ab2eae87feabd6497dc7348922ea37500f3d4dd690b65c900d33672cbe4c609b
relationship_secret[1]    = f1333fea95b9afed93d507bd6b5dc880e20348d7c65ab3992c7fa375236f9ef7
```

The complete vector includes both signatures and all four generation-bound chain
genesis digests. The generator verifies the signatures under the old
relationship keys and checks that each new directional secret differs from its
pre-reset input and from the opposite direction.

### 12.12 What Cannot Be Pinned
