 ...
 * deployment accumulates, plus the expired rows awaiting maintenance. Seeding
 * only deliveries would measure an index that never competes with anything.
 */
function seedWarmWorkingSet(path) {
  const database = new DatabaseSync(path);
  applyV2SQLiteMigrations(database, 1_800_000_000);
  database.exec('BEGIN IMMEDIATE');
  try {
    seedCapabilities(database);
    seedControlEvents(database);
    seedDeliveries(database);
...
function seedCapabilities(database) {
...
function seedControlEvents(database) {
...
function seedDeliveries(database) {
...
/** Times 100 empty-inbox queries after one warm-up query and returns the p95. */
async function emptyInboxP95(repository) {
  const query = {
    relationshipId: 'missing-relationship',
    direction: 'inviter->invitee',
    dataSlots: [{ slot: bytes(16, 0xff), epoch: 20_000 }],
    controlSlots: [],
    now: 1_800_000_000,
  };
  await repository.queryInbox(query);
  const samples = [];
...
test('warm SQLite empty inbox p95 stays below the V2 25ms gate with a full metadata working set', async (t) => {
  ...
  seedWarmWorkingSet(join(root, 'v2', 'v2.sqlite'));

  await repository.initialize();
  const p95 = await emptyInboxP95(repository);
