...
test('capability discovery advertises only implemented features and atomic quotas', async () => {
  ...
  assert.deepEqual(body.get(2), [2, 3, 5, 6, 7, 9, 10, 11]);
  assert.equal(body.get(3).get(1), 104857600);
  assert.equal(body.get(3).get(10), 16782955);
  assert.equal(body.get(3).get(11), 1024);
  assert.equal(body.get(3).get(12), 1073741824);
  assert.equal(body.get(3).get(13), 3600);
